...
if ( ! function_exists( 'ouv_posted_by' ) ) :
	...
	function ouv_posted_by() {
		...
		/* translators: 1: posted by label, only visible to screen readers. 2: author link. 3: post author. */
			...
			esc_html__( 'Posted by', 'ouvrages-wp' ),
	...
	function ouv_entry_meta() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				/* translators: 1: posted in label, only visible to screen readers. 2: list of categories. */
					...
					esc_html__( 'Posted in', 'ouvrages-wp' ),
					$categories_list // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				);
			}

			/* translators: used between list items, there is a space after the comma. */
			$tags_list = get_the_tag_list( '', __( ', ', 'ouvrages-wp' ) );
			if ( $tags_list ) {
				printf(
				/* translators: 1: tags label, only visible to screen readers. 2: list of tags. */
					'<span class="sr-only">%1$s</span>%2$s',
					esc_html__( 'Tags:', 'ouvrages-wp' ),
		...
		edit_post_link(
			sprintf(
				wp_kses(
				/* translators: %s: Name of current post. Only visible to screen readers. */
					__( 'Edit <span class="sr-only">%s</span>', 'ouvrages-wp' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'ouv_the_posts_navigation' ) ) :
	...
	function ouv_the_posts_navigation() {
		the_posts_pagination(
			array(
				'mid_size'  => 2,
				'prev_text' => __( 'Newer posts', 'ouvrages-wp' ),
				'next_text' => __( 'Older posts', 'ouvrages-wp' ),
			)
		);
	}
endif;
